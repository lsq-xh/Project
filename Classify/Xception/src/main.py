...
def main():
    ...
    data_dir = r"E:/DataSets/Concrete Crack Images for Classification/"
    training_log_dir = r"E:/Code_Py/Xception/train_log/"
    test_image_dir = r"E:/Code_Py/Xception/test_image/"
    model_save_dir = r"E:/Code_Py/Xception/model_save/"
    image_shape = (229, 229, 3)
    batch_size = 40
    epochs = 300
    lr = 1e-5
    datasets = DataProcess(file_path=data_dir, image_shape=image_shape, batch_size=batch_size)
    model = XCEPTION(number_of_classes=datasets.get_object_class_and_number()[0], image_shape=image_shape).xception_net()
    ...
    if not os.path.exists(model_save_dir):
        if tf.config.list_physical_devices('GPU'):
            ...
            image_predict(model_dir=os.path.join(model_save_dir, "Xception.h5"), test_image_dir=test_image_dir, label_dict=label_dict, image_shape=image_shape)
        else:

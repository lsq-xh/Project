...
def main():
    ...
    training_log_dir = r"E:/Code_Py/Mobile/train_log/"
    test_image_dir = r"E:/Code_Py/Mobile/test_image/"
    model_save_dir = r"E:/Code_Py/Mobile/model_save/"
    ...
    model = MOBILE(number_of_classes=datasets.get_object_class_and_number()[0], image_shape=image_shape).mobile_net(version=3)
    ...
    if not os.path.exists(model_save_dir):
        if tf.config.list_physical_devices('GPU'):
            ...
            image_predict(model_dir=os.path.join(model_save_dir, "mobilenet.h5"), test_image_dir=test_image_dir, label_dict=label_dict, image_shape=image_shape)
        else:

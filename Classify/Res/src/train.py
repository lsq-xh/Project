...
class TrainEValuateModel:
    ...
    def train_and_evaluate_model(self):
        ...
        self.model.compile(optimizer=optimizers.Adam(learning_rate=self.learning_rate,beta_1=0.9, beta_2=0.999), loss='categorical_crossentropy', metrics=["Accuracy"])
        self.model.summary()
        history = self.model.fit(x=train_ds, epochs=self.epochs, validation_data=val_ds, callbacks=callbacks_list)
        self.model.save(os.path.join(self.model_save_dir, "res.h5"), save_format="tf")
